...
You can install the 'bcgroundwater' R `r emo::ji("package")` package directly from [GitHub](https://github.com/bcgov/bcgroundwater). To do so, you will 
...
with no gaps in the dates. `makeWellTS()` does this for you, removing consecutive missing values near the
...
We can categorize the observed long-term trend using the criteria developed for the [Environmental Reporting BC indicator on trends in groundwater levels](http://www.env.gov.bc.ca/soe/indicators/water/groundwater-levels.html): Stable, Increasing, Moderate Rate of Decline or Large Rate of Decline:
...
```{r, message=FALSE, warning=FALSE, fig.align='center'}
trend_plot <- gwl_area_plot(full_monthly_data, trend = trends$trend[1],
                          intercept = trends$intercept[1], sig = trends$sig[1], state = state,
