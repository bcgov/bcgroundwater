...
One example of how to generate a reproducible trend analysis on B.C. groundwater level data using the `bcgroundwater` R package. 

This example examines long-term trends in water level data from [British Columbia groundwater observation monitoring well #45](https://www2.gov.bc.ca/gov/content?id=2D3DB9BA78DE4377AB4AE2DCEE1B409B), using both the Mann-Kendall and Seasonal Kendall non-parametric trend tests, implemented in the [`zyp`](https://cran.r-project.org/web/packages/zyp/index.html) and [`EnvStats`](https://cran.r-project.org/web/packages/EnvStats/index.html) R packages respectively.
